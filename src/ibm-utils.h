...
/*
void ibm_dirichlet_gradient (Point point, vector u, scalar ibm, coord n, coord pm, coord vb, coord * val)
{
    ...
    for (int l = 0; l <= 1; l++) {
        if (fabs(n.x) >= fabs(n.y)) {
            ...
            if (ibm[i*(l + 1), j] < 0.5 && ibm[i*(l + 1), j+1] < 0.5 && ibm[i*(l + 1), j-1] < 0.5)
                foreach_dimension()
                    v[l].x = quadratic(y1, (u.x[i*(l+1),j]), (u.x[i*(l+1),j+1]), (u.x[i*(l+1), j-1]));
            else
                foreach_dimension()
                    v[l].x = 0.;
            printy (point, ibm, u, n, d, v, x1, y1, i, j, l, pm);
        }
        else if (fabs(n.x) < fabs(n.y)) {
            int j = sign(n.y);
            y1 = (Delta)*((l + 1) - pm.y);
            ...
            if (ibm[i, j*(l + 1)] < 0.5 && ibm[i+1,j*(l + 1)] < 0.5 && ibm[i-1,j*(l + 1)] < 0.5)
                foreach_dimension()
    ...
    *val = gradient;
    printx (point, ibm, u, n, d, v, gradient);
}


coord ibm_gradient (Point point, vector u, coord pm, coord n, coord vb, scalar ibm)
{
    coord dudn;
    ibm_dirichlet_gradient (point, u, ibm, n, pm, vb, &dudn);
    if (dudn.x == nodata)
        dudn.x = -0.;
    return dudn;
}
...
/*
foreach_dimension()
static inline double dirichlet_gradient_x (Point point, scalar s, scalar ibm, face vector sf,
...
double dirichlet_gradient (Point point, scalar s, scalar ibm, face vector sf,
			               coord n, coord p, double bc, double * coef)
{
#if dimension == 2
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return dirichlet_gradient_x (point, s, ibm, sf, n, p, bc, coef);
#endif
  return nodata;
}
*/
